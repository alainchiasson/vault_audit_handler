#!/usr/bin/env python3
"""Print to console all events where `type == 'request'`."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Print all 'request' events to stdout")
    parser.add_argument("path", help="Path to the audit log file (.private_log or .gz)")
    args = parser.parse_args()

    reader = VaultLogReader(args.path)
    filt = VaultEventFilter("type", "request")
